...
def obtener_vecinos(conexion):
    ...
    for coincidencia in patron.finditer(salida):
        lista_vecinos.append({
            ...
            "interfaz": coincidencia.group(3).lower().replace("fastethernet", "fa").replace("gigabitethernet", "gi")
        })
...
if __name__ == "__main__":
    print(" identificando switches Cisco...\n")
    ips_disponibles = obtener_ips_arp()
    lista_switches = identificar_switches(ips_disponibles)

    if not lista_switches:
        print("No se detectaron switches Cisco en la red.")
        exit()

    print(f"Se detectaron {len(lista_switches)} switches Cisco listos para analizar.\n")
    ...
    while True:
        ...
        if resultado:
            print("\nHOST DETECTADO:")
            print(f"   Switch: {resultado['switch']}")
